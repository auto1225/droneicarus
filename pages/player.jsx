...
function PlayerPage({ video, onNav, onOpenVideo }) {
  ...
  const loc = window.LOCATIONS.find(l => l.id === video.locationId);
  const related = window.VIDEOS.filter(v => v.locationId === video.locationId && v.id !== video.id).slice(0, 8);
          ...
          {locked && (
            <>
              <div style={{
                ...
                background: window.thumbGradient(parseInt(video.id.slice(1))),
              }}>
        ...
        <div style={{ marginTop: 20 }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
            <span className="chip" style={{ padding: '4px 10px', fontSize: 12 }}>
              <span className="chip-icon" style={{ display: 'inline-flex' }}>{window.CAT_ICONS[video.category] && window.CAT_ICONS[video.category](12)}</span>
              {window.CATEGORIES.find(c => c.id === video.category)?.label}
            </span>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 20, paddingBottom: 20, borderBottom: '1px solid var(--line)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--parchment-dim)' }}>{video.creator.handle} · {window.formatViews(128000)} followers</div>
              </div>
              <button className="btn" style={{ marginLeft: 12, padding: '8px 18px', fontSize: 13 }}>Follow</button>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="btn secondary" style={{ fontSize: 13 }}><Ic.heart/> {window.formatViews(video.likes)}</button>
          ...
          <div style={{ display: 'flex', gap: 0, marginTop: 20, border: '1px solid var(--line)', borderRadius: 4 }}>
            {[
              ['Views', window.formatViews(video.views)],
              ['Uploaded', window.formatDays(video.uploadedDaysAgo)],
          ...
          {related.map(v => (
            <div key={v.id} onClick={() => onOpenVideo(v)} style={{ display: 'flex', gap: 10, cursor: 'pointer' }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--parchment-dim)' }}>{window.formatViews(v.views)} views · {window.formatDays(v.uploadedDaysAgo)}</div>
              </div>
            </div>
          ))}
        </div>
