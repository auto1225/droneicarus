        ...
        <div style={{ padding: '22px 22px 14px', borderBottom: '1px solid var(--line)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
            ...
            <button onClick={() => auth.user ? setGoLiveOpen(true) : onNav?.('signin')}
              style={{ padding: '8px 14px', background: 'var(--sunset)', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer', fontWeight: 700, fontSize: 14, letterSpacing: '0.05em' }}>
              ● Go Live
            </button>
          </div>
          <div style={{ display: 'flex', gap: 4, marginTop: 14 }}>
            {[['all','All'],['broadcasting','On air · '+broadcasting.length],['uploading','Uploading · '+uploading.length]].map(([k,l]) => (
              <button key={k} onClick={() => setTab(k)} className={'chip' + (tab === k ? ' active' : '')} style={{ fontSize: 12 }}>{l}</button>
            ))}
          </div>
        </div>
            ...
            <button key={l.id} onClick={() => setSelected(l)} style={{
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--parchment-dim)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: 4 }}>{l.place}</div>
                  {l.kind === 'broadcasting' ? (
                    <div className="mono" style={{ fontSize: 12, color: 'var(--sunset)', letterSpacing: '0.08em' }}>● {l.viewers.toLocaleString()} WATCHING · {l.started}</div>
                  ) : (
                    <div>
                      <div className="mono" style={{ fontSize: 12, color: 'var(--amber)', letterSpacing: '0.08em', marginBottom: 3 }}>↑ UPLOADING {l.progress}%</div>
      ...
      <main style={{ padding: 28, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 18 }}>
          <div>
            ...
            <div style={{ fontSize: 14, color: 'var(--parchment-dim)' }}>Pulsing dots are pilots broadcasting live · Click to preview</div>
          </div>
          <div className="mono" style={{ fontSize: 12, letterSpacing: '0.14em', color: 'var(--lichen)' }}>● 8 / 8 FEEDS HEALTHY</div>
        </div>
          ...
          <>
            <div style={{ aspectRatio: '16/9', background: '#0a0d0a', borderRadius: 3, position: 'relative', marginBottom: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', border: '1px solid var(--line-strong)' }}>
              ...
              {(selected.kind === 'broadcasting' || selected.status === 'live') ? (
                <>
                  <span style={{ padding: '4px 9px', background: 'var(--sunset)', color: '#faf6ec', fontSize: 12, fontFamily: 'var(--font-mono)', letterSpacing: '0.14em', borderRadius: 2, position: 'absolute', top: 10, left: 10, fontWeight: 700 }}>● LIVE</span>
                  <span style={{ position: 'absolute', bottom: 10, right: 10, padding: '3px 7px', background: 'rgba(13,20,16,0.85)', color: '#f5ede0', fontSize: 12, fontFamily: 'var(--font-mono)', borderRadius: 2, backdropFilter: 'blur(4px)' }}>{(selected.viewers || 0).toLocaleString()} watching</span>
                ...
                <>
                  <span style={{ padding: '4px 9px', background: 'var(--amber)', color: '#1a2820', fontSize: 12, fontFamily: 'var(--font-mono)', letterSpacing: '0.14em', borderRadius: 2, position: 'absolute', top: 10, left: 10, fontWeight: 700 }}>↑ UPLOADING</span>
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 12, background: 'linear-gradient(0deg, rgba(13,20,16,0.95), transparent)' }}>
                    <div style={{ fontSize: 12, fontFamily: 'var(--font-mono)', color: 'var(--amber)', marginBottom: 6 }}>{selected.progress}% · {(selected.progress * 0.65).toFixed(1)} GB / {(100 * 0.65).toFixed(1)} GB</div>
            ...
            <div style={{ fontSize: 14, color: 'var(--parchment-dim)', marginBottom: 14 }}>{selected.pilot} · {selected.handle}</div>
            <div style={{ display: 'flex', gap: 6, marginBottom: 18 }}>
              <button className="btn" style={{ fontSize: 14, flex: 1 }} data-placeholder="true">Tune in</button>
              {(selected.id && selected.id.length === 36 && selected.monetization_enabled) && <button className="btn secondary" style={{ fontSize: 14 }} onClick={() => setTipOpen(true)}>Tip $</button>}
              {(selected.id && selected.id.length === 36 && auth.user && auth.user.id === selected.pilot_id && selected.status === 'live') &&
                <button className="btn secondary" style={{ fontSize: 14, color: 'var(--sunset)', borderColor: 'var(--sunset)' }} onClick={async () => {
            ...
            {selected.id && selected.id.length === 36 ? (
              ...
              <div style={{ padding: 14, background: 'var(--forest-900)', border: '1px dashed var(--line)', borderRadius: 4, color: 'var(--parchment-dim)', fontSize: 14 }}>
            ...
            {[['Altitude', '84 m'], ['Distance', '1.2 km'], ['Battery', '56%'], ['Wind', '12 km/h NW']].map(([k, v]) => (
              <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: 14, borderTop: '1px solid var(--line)' }}>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--ink)', padding: 26, borderRadius: 8, maxWidth: 540, width: '92%', border: '1px solid var(--line-strong)', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ fontSize: 12, color: 'var(--sunset)', fontFamily: 'var(--font-mono)', letterSpacing: '0.18em', marginBottom: 6 }}>● GO LIVE · YouTube mirror</div>
        <h3 style={{ marginTop: 0, marginBottom: 8 }}>Start a live broadcast</h3>
        <p style={{ fontSize: 14, color: 'var(--parchment-dim)', marginBottom: 14, lineHeight: 1.5 }}>
          droneicarus mirrors your live stream from YouTube. Chat & Super Chat run on our site (you keep 70%). <button type="button" onClick={() => { onClose(); onNav?.('livehelp'); }} style={{ background: 'transparent', border: 'none', color: 'var(--amber)', cursor: 'pointer', textDecoration: 'underline', padding: 0, fontSize: 14 }}>Full guide →</button>
        </p>

        <div style={{ marginBottom: 16, padding: '12px 14px', background: 'var(--forest-900)', border: '1px solid var(--line)', borderRadius: 4, fontSize: 14 }}>
          ...
          <ol style={{ margin: 0, paddingLeft: 20, color: 'var(--parchment)', lineHeight: 1.7 }}>
            ...
            <li>Copy the URL (e.g. <code style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--amber)' }}>youtube.com/watch?v=...</code>)</li>
            <li>Paste below + press Start. Viewers watch the YT player here, chat with you in our chat panel.</li>
          </ol>
        </div>
        <div style={{ marginBottom: 12 }}>
          <label style={{ display: 'block', fontSize: 12, color: 'var(--parchment-dim)', marginBottom: 4 }}>Stream title *</label>
        ...
        <div style={{ marginBottom: 12 }}>
          <label style={{ display: 'block', fontSize: 12, color: 'var(--parchment-dim)', marginBottom: 4 }}>Description</label>
          <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2}
            style={{ width: '100%', padding: '10px 12px', background: 'var(--forest-900)', border: '1px solid var(--line-strong)', color: 'var(--bone)', borderRadius: 4, fontFamily: 'inherit', fontSize: 14 }}/>
        </div>
        <div style={{ marginBottom: 12 }}>
          <label style={{ display: 'block', fontSize: 12, color: 'var(--parchment-dim)', marginBottom: 4 }}>YouTube Live URL *</label>
          <input value={ytUrl} onChange={e => setYtUrl(e.target.value)} placeholder="https://youtube.com/watch?v=..."
            style={{ width: '100%', padding: '10px 12px', background: 'var(--forest-900)', border: '1px solid var(--line-strong)', color: 'var(--bone)', borderRadius: 4, fontFamily: 'inherit' }}/>
          <div style={{ fontSize: 12, color: 'var(--parchment-dim)', marginTop: 4 }}>Required. The 11-char video id is auto-extracted from any youtube.com / youtu.be URL.</div>
        </div>
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: 'block', fontSize: 12, color: 'var(--parchment-dim)', marginBottom: 4 }}>Custom thumbnail URL (optional)</label>
        ...
        <div style={{ marginBottom: 14, padding: 14, background: monetize ? 'rgba(198,136,32,0.06)' : 'var(--forest-900)', border: '1px solid ' + (monetize ? 'var(--amber)' : 'var(--line)'), borderRadius: 4 }}>
          <label style={{ display: 'flex', gap: 10, alignItems: 'flex-start', cursor: 'pointer' }}>
            ...
            <span style={{ flex: 1 }}>
              <span style={{ fontSize: 14, fontWeight: 600, display: 'block' }}>Enable Super Chat tipping (monetization)</span>
              <span style={{ fontSize: 14, color: 'var(--parchment-dim)' }}>Viewers can send tips $1–$100. You keep 70%, platform 30%.</span>
              {payoutReady === false && (
                <span style={{ display: 'block', marginTop: 8, padding: '8px 10px', background: 'rgba(217,112,69,0.1)', border: '1px solid var(--sunset)', borderRadius: 3, fontSize: 12 }}>
              ...
              {payoutReady === null && <span style={{ display: 'block', marginTop: 6, fontSize: 12, color: 'var(--parchment-dim)' }}>Checking your payout setup…</span>}
              {payoutReady === true && <span style={{ display: 'block', marginTop: 6, fontSize: 12, color: 'var(--moss)' }}>✓ Payout profile ready</span>}
            </span>
          </label>
        </div>
        {err && <div style={{ color: 'var(--sunset)', fontSize: 14, marginBottom: 12 }}>{err}</div>}
