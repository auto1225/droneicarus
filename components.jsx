...
function SearchDropdown({ query, onNav, onSelect, onClose }) {
  const [active, setActive] = useState(0);
  const q = query.trim().toLowerCase();

  const locMatches = q ? window.LOCATIONS.filter(l =>
    l.name.toLowerCase().includes(q) || l.country.toLowerCase().includes(q)
  ).slice(0, 5) : [];
  const creatorMatches = q ? window.VIDEOS
    ...
    .slice(0, 3) : [];
  const videoMatches = q ? window.VIDEOS.filter(v =>
        ...
        <>
          ...
          {videoMatches.map(v => (
            <div key={v.id} className="search-dd-item" onClick={() => { onSelect?.({ type: 'video', data: v }); onClose(); }}>
              <span className="icon" style={{ width: 48, height: 30, background: window.thumbGradient(parseInt(v.id.slice(1))), color: '#faf6ec' }}>
                <Ic.play size={11}/>
              </span>
    ...
    <div style={{ position: 'relative', padding: compact ? '10px 0' : '16px 0' }}>
      <div ref={scrollRef} style={{
        ...
        {window.CATEGORIES.map(c => {
          const Icon = window.CAT_ICONS[c.id];
          return (
...
// ——— Video Card ———
function VideoCard({ video, onClick, size = 'md', showRank = false }) {
...
Object.assign(window, { Ic, formatViews, formatDays, Header, CategoryChips, VideoCard, Footer, SearchDropdown });
