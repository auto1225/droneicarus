...
const { useState: ckUseState, useEffect: ckUseEffect } = React;

function CheckoutPage({ videoId, licenseType = 'Commercial', onNav }) {
  const v = window.VIDEOS.find(x => x.id === videoId) || window.VIDEOS.find(x => x.price > 0);
  ...
  const loc = window.LOCATIONS.find(l => l.id === v.locationId);
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      ...
      window.__lastOrder = { id: orderId, videoId: v.id, tier, subtotal: sub, tax, total, method: payMethod === 'paypal' ? 'PayPal · ' + email : 'Visa •• ' + card.slice(-4) };
      onNav('success');
    }, 1600);
  };
    ...
    <div style={{ background: 'var(--ink)', minHeight: 'calc(100vh - 62px)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 28px 60px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 72 }}>
        ...
        <div>
          ...
          <div style={{
            aspectRatio: '16/9', borderRadius: 6, background: window.thumbGradient(v.id.charCodeAt(1) || 5),
...
function SuccessPage({ onNav }) {
  const order = window.__lastOrder || { id: 'DI-2026-04821', videoId: 'v0', tier: 'Commercial', total: 17.98, method: 'Visa •• 4242' };
  const v = window.VIDEOS.find(x => x.id === order.videoId) || window.VIDEOS[0];
    ...
    <div style={{ maxWidth: 780, margin: '0 auto', padding: '60px 28px' }}>
      ...
      <div style={{ border: '1px solid var(--line-strong)', borderRadius: 6, overflow: 'hidden', marginBottom: 22 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: 0 }}>
          <div style={{ aspectRatio: '16/9', background: window.thumbGradient(v.id.charCodeAt(1) || 2), position: 'relative' }}>
      ...
      <div style={{ marginTop: 40, padding: '18px 20px', background: 'var(--forest-900)', border: '1px solid var(--line)', borderRadius: 4, display: 'flex', gap: 14, alignItems: 'center' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--parchment-dim)' }}>{v.creator.name} ({v.creator.handle}) receives 85% of every license. Thanks for supporting pilots directly.</div>
        </div>
      </div>
    </div>
  );
...
function OrdersPage({ onNav }) {
  ...
  const rows = filter === 'all' ? window.ORDERS : window.ORDERS.filter(o => o.license.toLowerCase() === filter);
  const lifetime = window.ORDERS.reduce((s, o) => s + o.total, 0);
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 28px 80px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0, border: '1px solid var(--line)', borderRadius: 4, marginBottom: 28 }}>
        {[
          ...
          ['Licenses held', window.ORDERS.length],
          ['Pilots supported', new Set(window.ORDERS.map(o => window.VIDEOS.find(v => v.id === o.videoId)?.creator.handle)).size],
      ...
      <div style={{ border: '1px solid var(--line)', borderRadius: 4 }}>
        ...
        {rows.map((o, i) => {
          const v = window.VIDEOS.find(x => x.id === o.videoId);
          return (
            <div key={o.id} style={{ display: 'grid', gridTemplateColumns: '140px 2fr 1fr 1fr 110px 130px', gap: 14, padding: '16px 18px', alignItems: 'center', borderBottom: i < rows.length - 1 ? '1px solid var(--line)' : 'none' }}>
              ...
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <div style={{ width: 56, height: 32, borderRadius: 2, background: window.thumbGradient(i + 2), flexShrink: 0 }}/>
...
function LicenseDetailPage({ orderId, onNav }) {
  const o = window.ORDERS.find(x => x.id === orderId) || window.ORDERS[0];
  const v = window.VIDEOS.find(x => x.id === o.videoId);
  const loc = window.LOCATIONS.find(l => l.id === v?.locationId);
  return (
        ...
        <div style={{ padding: 48, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48 }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
              <div style={{ width: 80, height: 50, background: window.thumbGradient(5), borderRadius: 2, flexShrink: 0 }}/>
              <div>
          ...
          <div>
            ...
            <Kv k="Name" v={window.CURRENT_USER.name}/>
            <Kv k="Email" v={window.CURRENT_USER.email}/>
            <Kv k="User ID" v={window.CURRENT_USER.id}/>
...
Object.assign(window, { CheckoutPage, SuccessPage, OrdersPage, LicenseDetailPage });
