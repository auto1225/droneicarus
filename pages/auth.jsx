  ...
  return (
    <label style={{ display: 'block', marginBottom: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
        ...
        {hint && <button type="button" style={{ fontSize: 11, color: 'var(--sunset)' }}>{hint}</button>}
      </div>
      <div style={{ position: 'relative' }}>
        {children}
        {success && (
...
function AuthPage({ onNav }) {
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      if (mode === 'signin') { window.toast?.('Welcome back', 'Signed in to Drone Icarus'); onNav('home'); }
      else if (mode === 'signup') { setStep(2); }
      else if (mode === 'forgot') { setMode('reset-sent'); }
    }, 700);
  };
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 22 }}>
              ...
              <button className="btn primary" disabled={!name || handle.length < 3} onClick={() => {
                window.toast?.('Welcome aboard, ' + (name || 'pilot') + '!', 'Account created');
