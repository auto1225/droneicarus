...
const { useState: uUseState, useEffect: uUseEffect, useMemo: uUseMemo, useRef: uUseRef } = React;
...
function UploadPage({ onNav }) {
  ...
  const loc = window.LOCATIONS.find(l => l.id === locationId);
      ...
      <div style={{ maxWidth: 1120, margin: '0 auto', padding: '28px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24, marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button className="btn" disabled={!canPublish}
              onClick={() => { alert('Published to Drone Icarus · awaiting review'); onNav('creator'); }}
              style={{ padding: '10px 18px', fontSize: 13, opacity: canPublish ? 1 : 0.45 }}>
              Publish
            </button>
          </div>
        </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                    {['0:12', '2:34', '5:08'].map((ts, i) => (
                      <div key={i} style={{
                        ...
                        background: window.thumbGradient(i + 3),
                ...
                <Field label="Location">
                  <select value={locationId} onChange={e => setLocationId(e.target.value)} style={fieldStyle}>
                    {window.LOCATIONS.map(l => <option key={l.id} value={l.id}>{l.name} · {l.country}</option>)}
                  </select>
                </Field>
          ...
          <aside>
            <div style={{ position: 'sticky', top: 90 }}>
              <div style={{ border: '1px solid var(--line-strong)', borderRadius: 6, overflow: 'hidden' }}>
                <div style={{ aspectRatio: '16/9', background: window.thumbGradient(3), position: 'relative' }}>
