...
function EarningsPage({ onNav }) {
  const [period, setPeriod] = eUseState('30d');
  const payouts = window.PAYOUTS || [];
  const myVids = window.VIDEOS.filter(v => v.price > 0).slice(0, 6);
          ...
          <div style={{ border: '1px solid var(--line)', borderRadius: 4 }}>
            ...
            {myVids.map((v, i) => (
              <div key={v.id} style={{ display: 'grid', gridTemplateColumns: '30px 56px 1fr 90px 90px', gap: 12, padding: '12px 20px', borderTop: i === 0 ? 'none' : '1px solid var(--line)', alignItems: 'center' }}>
                ...
                <div style={{ aspectRatio: '16/9', background: window.thumbGradient(parseInt(v.id.slice(1))), borderRadius: 2 }}/>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--parchment-dim)' }}>gross</div>
                </div>
...
function SidebarPilot({ active, onNav }) {
...
Object.assign(window, { EarningsPage, SidebarPilot });
