...
function ShotLibraryPage({ onNav, onOpenVideo }) {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 22 }}>
          {[
            ...
            <div key={k.name} style={{ border: '1px solid var(--line)', borderRadius: 4, overflow: 'hidden', display: 'grid', gridTemplateColumns: '200px 1fr' }}>
              <div style={{ position: 'relative', background: window.thumbGradient(k.name.charCodeAt(0) + i) }}>
                <div style={{ position: 'absolute', inset: 0, display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr', gap: 2, padding: 2 }}>
                  {[0,1,2,3].map(j => <div key={j} style={{ background: window.thumbGradient(k.name.charCodeAt(0) + i + j + 2), opacity: 0.8 }}/>)}
                </div>
              </div>
...
Object.assign(window, { ShotLibraryPage });
