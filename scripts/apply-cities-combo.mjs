...
const rows=JSON.parse(fs.readFileSync('scripts/cities-batch-combo.json','utf8'));
console.log(`${rows.length} city videos to insert...`);
let ok=0, fail=0;
for(const row of rows){
  const r=await fetch(`${SUPA}/rest/v1/videos`,{method:'POST',headers:H,body:JSON.stringify(row)});
  if(r.ok)ok++; else{fail++; if(fail<3)console.warn(`FAIL ${r.status}`);}
}
