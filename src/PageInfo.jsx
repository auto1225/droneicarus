...
const INFO = {
  home: {
    ...
    sections: [
      { kind: 'overview', text: '드론 이카루스의 메인 시작점입니다. 일반 취미 촬영자부터 전문 영상가까지 전 세계 다양한 드론 크리에이터가 올린 1,650개 이상의 영상이 촬영 위치(좌표) 기준으로 지도 위 핀으로 표시됩니다. 사이드바와 필터로 좁혀가며 원하는 장면을 발견하고, 핀 클릭 한 번이면 영상 페이지로 진입.' },
  ...
  watch: {
    ...
    sections: [
      ...
      { kind: 'tips', items: [
        ...
        '제작자 핸들 클릭 → 그 크리에이터의 모든 작품 보기 (취미 촬영자·전문 파일럿 모두 환영).',
  ...
  commissions: {
    ...
    sections: [
      ...
      { kind: 'glossary', title: '4단계 흐름', items: [
        ...
        { term: 'Step 2', desc: '입찰 받기 — 24~72h 내 첫 입찰. 각 크리에이터의 프로필·평점·과거 작품 검토. Verified 마크가 있으면 신뢰도 ↑, 없어도 평점 좋으면 OK.' },
      ...
      { kind: 'faq', items: [
        ...
        { q: '입찰자는 모두 검증된 파일럿인가요?', a: '아니요. 일반 가입자도 입찰 가능합니다. 단 의뢰자가 입찰자별 평점·과거 작품·Verified 여부를 보고 직접 선택. 안전 우려가 있으면 Verified만 모집 옵션도 가능.' },
        { q: '파일럿이 도중에 잠적하면?', a: '마감일 + 7일 이내 결과물이 안 오면 자동 환불 처리됩니다. 잠적한 크리에이터는 사이트에서 추방.' },
      ]},
    ],
  ...
  upload: {
    ...
    sections: [
      ...
      { kind: 'tips', items: [
        ...
        '가입 자체에는 면허·인증 불필요. 일반 가입자도 즉시 업로드·판매 가능.',
        'Verified 마크는 옵션 — 신원 + 포트폴리오 검수 통과 시 부여 (구매자 신뢰도 ↑, 판매가 평균 +30%).',
      ]},
      { kind: 'faq', items: [
        { q: '항공 파일럿 면허가 없는데 등록 가능한가요?', a: '네, 누구나 가능합니다. 자국 드론 등록·기본 안전 수칙만 지키면 OK (한국은 250g 이상 신고제, 미국은 FAA 등록). 항공 파일럿 면허는 BVLOS·야간·인구 밀집 지역 같은 특수 비행에만 필요해요.' },
  ...
  creators: {
    ...
    sections: [
      ...
      { kind: 'glossary', title: '파일럿 등급', items: [
        { term: '✓ Verified', color: '#c68820', desc: '신원 인증 + 포트폴리오 품질 검수 통과 (옵션). 항공 파일럿 면허는 필수 아님 — 자국 드론 등록만 준수하면 OK.' },
      ...
      { kind: 'faq', items: [
        { q: '"Verified" 마크는 어떤 기준?', a: '신원 인증(여권/주민증) + 포트폴리오 검수(영상 품질·일관성)만. 항공 파일럿 면허는 필요 없어요 — 자국 드론 등록만 지키면 됩니다. 일반 사용자도 가입 즉시 업로드·판매 가능, Verified는 신뢰도 부여 옵션.' },
  ...
  atlas: {
    ...
    sections: [
      ...
      { kind: 'glossary', title: '의뢰 카드 영역 안내', items: [
        ...
        { term: 'Claim 버튼', desc: '"내가 도전한다" 선언. Easy/Moderate는 누구나, Hard/Extreme는 Verified 크리에이터 권장. 30일 내 영상 제출 의무.' },
