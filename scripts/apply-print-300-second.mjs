...
const items = JSON.parse(fs.readFileSync('scripts/print-300-second.json','utf8'));
...
for (const it of items) {
  ...
  // Ensure uniqueness by adding Printables model ID if slug collides
  if (seenSlugs.has(s)) {
    const idMatch = it.url.match(/\/model\/(\d+)-/);
    if (idMatch) s = `${s}-${idMatch[1]}`;
    else { dup++; continue; }
  }
  seenSlugs.add(s);
  const body = {
  ...
  if (r.ok) ok++;
  else { fail++; if (fail<5) console.warn(`FAIL ${s}: ${r.status} ${await r.text()}`); }
}
