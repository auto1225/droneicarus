...
function RankingsPage({ onOpenVideo }) {
  ...
  return (
    <div style={{ maxWidth: 1760, margin: '0 auto', padding: '40px 28px 60px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.15fr 1fr', gap: 16, marginBottom: 40, alignItems: 'end' }}>
        {[window.TRENDING[1], window.TRENDING[0], window.TRENDING[2]].map((v, i) => {
          ...
          return (
            <div key={v.id} onClick={() => onOpenVideo(v)} style={{
              ...
              <div style={{
                position: 'relative',
                aspectRatio: '16/9',
                background: window.thumbGradient(parseInt(v.id.slice(1))),
              ...
              <div style={{ fontSize: 12, color: 'var(--parchment-dim)' }}>{v.creator.handle} · {window.formatViews(v.views)} views</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ border: '1px solid var(--line)', borderRadius: 4, overflow: 'hidden' }}>
        ...
        {window.TRENDING.map((v, i) => {
          ...
          return (
            <div key={v.id} onClick={() => onOpenVideo(v)} style={{
              display: 'grid', gridTemplateColumns: '60px 1.2fr 2fr 1fr 1fr 120px 100px',
              padding: '16px 20px', borderBottom: i === window.TRENDING.length - 1 ? 'none' : '1px solid var(--line)',
              alignItems: 'center',
              cursor: 'pointer',
              ...
              <div style={{ display: 'flex', gap: 12, alignItems: 'center', minWidth: 0 }}>
                <div style={{ width: 80, aspectRatio: '16/9', background: window.thumbGradient(parseInt(v.id.slice(1))), borderRadius: 2, flexShrink: 0, border: '1px solid var(--line)' }}/>
              ...
              <span className="mono" style={{ textAlign: 'right', fontSize: 13 }}>{window.formatViews(v.views)}</span>
...
function CreatorsPage({ onOpenVideo }) {
  return (
    <div style={{ maxWidth: 1760, margin: '0 auto', padding: '40px 28px 60px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 20 }}>
        {window.CREATORS.map(c => {
          const theirVids = window.VIDEOS.filter(v => v.creator.handle === c.handle).slice(0, 3);
          return (
            <div key={c.handle} style={{ padding: 24, background: 'var(--forest-900)', border: '1px solid var(--line)', borderRadius: 4 }}>
              ...
              <div style={{ display: 'flex', gap: 20, marginBottom: 18, paddingBottom: 18, borderBottom: '1px solid var(--line)' }}>
                <div>
                  ...
                  <div style={{ fontSize: 18, fontFamily: 'var(--font-display)', fontWeight: 600 }}>{window.formatViews(c.followers)}</div>
                </div>
                <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {theirVids.map(v => (
                  <div key={v.id} onClick={() => onOpenVideo(v)} style={{ cursor: 'pointer', aspectRatio: '16/9', background: window.thumbGradient(parseInt(v.id.slice(1))), borderRadius: 2, position: 'relative', border: '1px solid var(--line)' }}>
...
Object.assign(window, { RankingsPage, CreatorsPage });
