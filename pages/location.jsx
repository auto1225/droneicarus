...
function LocationPage({ id, onOpenVideo, onNav }) {
  const loc = window.LOCATIONS.find(l => l.id === id) || window.LOCATIONS[0];
  const vids = window.VIDEOS.filter(v => v.locationId === loc.id);
  const display = vids.length ? vids : window.VIDEOS.slice(0, 12);

  return (
    <div>
      {/* Parallax-style hero */}
      <header style={{ position: 'relative', height: 560, overflow: 'hidden', background: window.thumbGradient(loc.id.charCodeAt(0)), borderBottom: '1px solid var(--line)' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '50px 28px', display: 'grid', gridTemplateColumns: '1.7fr 1fr', gap: 50 }}>
        ...
        <aside>
          ...
          <div style={{ aspectRatio: '4/3', background: window.thumbGradient(loc.id.charCodeAt(1) + 3), borderRadius: 3, marginBottom: 10 }}/>
