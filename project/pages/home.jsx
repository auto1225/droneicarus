...
function MapHero({ selectedLoc, onSelectLoc, categoryFilter, mapFilters }) {
  ...
  hUseEffect(() => {
    ...
    const addPin = (loc, selected) => {
      ...
      const vids = window.VIDEOS.filter(v => v.locationId === loc.id).slice(0, 3);
      ...
      const tipHtml = `
        <div class="pin-tip">
          <div class="pin-tip-thumbs">
            ${vids.map((v,i) => `<div class="pin-tip-thumb" style="background:${hEsc(window.thumbGradient(parseInt(v.id.slice(1))))};">
  ...
  const randomLand = () => {
    const map = mapInstance.current;
    if (!map) return;
    const pool = filteredLocs.length ? filteredLocs : window.LOCATIONS;
  ...
  return (
    <div style={{ position: 'relative', height: 'calc(100vh - 62px)', minHeight: 560 }}>
      ...
      <div style={{
        ...
        <div className="eyebrow" style={{ marginBottom: 14, color: 'var(--sunset)' }}>● Live · {filteredLocs.length} of {window.LOCATIONS.length} landmarks shown</div>
