...
function ProfilePage({ handle, onOpenVideo, onNav }) {
  const creator = (window.CREATORS || []).find(c => c.handle === handle || c.handle === '@' + handle) || window.CREATORS[0];
  const [tab, setTab] = pUseState('clips');
  const vids = window.VIDEOS.filter(v => v.creator.handle === creator.handle || v.creator.name === creator.name);
  const displayVids = vids.length ? vids : window.VIDEOS.slice(0, 12);
  ...
  return (
    <div>
      {/* Hero banner */}
      <div style={{ position: 'relative', height: 380, overflow: 'hidden', background: window.thumbGradient(displayVids[0]?.id.charCodeAt(1) || 3), borderBottom: '1px solid var(--line)' }}>
              ...
              {['Coastal golden hour', 'Seoul density', 'The blue hour', 'Industrial harbors', 'Mountain weather', 'Waterfalls of Jeju'].map((name, i) => (
                <div key={name} style={{ cursor: 'pointer' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gridTemplateRows: '1fr 1fr', gap: 3, aspectRatio: '4/3', borderRadius: 4, overflow: 'hidden', marginBottom: 10 }}>
                    <div style={{ gridRow: 'span 2', background: window.thumbGradient(i * 3) }}/>
                    <div style={{ background: window.thumbGradient(i * 3 + 1) }}/>
                    <div style={{ background: window.thumbGradient(i * 3 + 2) }}/>
                  </div>
