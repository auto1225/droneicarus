...
function CreatorDashboard({ onNav }) {
  const [step, setStep] = React.useState('dashboard'); // dashboard | upload
  const myVids = window.VIDEOS.slice(0, 8);
          ...
          {myVids.map(v => {
            ...
            return (
              <div key={v.id} style={{ display: 'grid', gridTemplateColumns: '60px 2fr 1fr 90px 90px 120px 100px', padding: '12px 20px', borderTop: '1px solid var(--line)', alignItems: 'center' }}>
                <div style={{ width: 44, aspectRatio: '16/9', background: window.thumbGradient(parseInt(v.id.slice(1))), borderRadius: 2, border: '1px solid var(--line)' }}/>
                <div>
                ...
                <span className="mono" style={{ textAlign: 'right', fontSize: 13 }}>{window.formatViews(v.likes)}</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <FormGroup label="Category">
              <select value={cat} onChange={e => setCat(e.target.value)} style={inputStyle}>
                {window.CATEGORIES.slice(1).map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
              </select>
            </FormGroup>
