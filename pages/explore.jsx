...
function ExplorePage({ onOpenVideo, onNav }) {
  const [active, setActive] = React.useState('all');
  const vids = active === 'all' ? window.VIDEOS : window.VIDEOS.filter(v => v.category === active);

  // Category cells
  const catStats = window.CATEGORIES.slice(1).map(c => ({
    ...c,
    count: window.VIDEOS.filter(v => v.category === c.id).length,
  }));
  ...
  return (
    <div style={{ maxWidth: 1760, margin: '0 auto', padding: '40px 28px 60px' }}>
      ...
      <p style={{ fontSize: 16, color: 'var(--parchment)', maxWidth: 640, marginBottom: 40 }}>
        ...
        Our taxonomy spans {window.CATEGORIES.length - 1} themes and counting.
      </p>
      ...
      <div style={{ borderTop: '1px solid var(--line)', paddingTop: 32 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', margin: '24px 28px 18px' }}>
          <h2 style={{ fontSize: 24 }}>{window.CATEGORIES.find(c => c.id === active)?.label} · <span className="mono" style={{ color: 'var(--parchment-dim)', fontWeight: 400 }}>{vids.length} clips</span></h2>
...
function SearchPage({ query, onOpenVideo, onNav, onSelectLoc }) {
  const q = (query || '').toLowerCase().trim();
  const matchedLocs = window.LOCATIONS.filter(l =>
    l.name.toLowerCase().includes(q) || l.country.toLowerCase().includes(q)
  );
  const matchedVideos = window.VIDEOS.filter(v =>
    v.title.toLowerCase().includes(q) ||
    v.creator.name.toLowerCase().includes(q) ||
    v.creator.handle.toLowerCase().includes(q) ||
    v.category.toLowerCase().includes(q)
  );
  const matchedCreators = window.CREATORS.filter(c =>
            ...
            {matchedCreators.map(c => (
              <div key={c.handle} style={{ padding: 16, background: 'var(--forest-900)', border: '1px solid var(--line)', borderRadius: 4, display: 'flex', gap: 12, alignItems: 'center' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--parchment-dim)' }}>{c.handle} · {window.formatViews(c.followers)} followers</div>
                </div>
...
Object.assign(window, { ExplorePage, SearchPage });
