...
function CollectionPage({ id, onOpenVideo, onNav }) {
  const col = window.COLLECTIONS.find(c => c.id === id) || window.COLLECTIONS[0];
  const videos = (col.cover || []).map(vid => window.VIDEOS.find(v => v.id === vid)).filter(Boolean);
  while (videos.length < 12) videos.push(window.VIDEOS[(videos.length * 5) % window.VIDEOS.length]);
  const author = window.CURRENT_USER;
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1.1fr', gap: 50, alignItems: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gridTemplateRows: '110px 110px 110px', gap: 5, height: 340 }}>
            {videos.slice(0, 9).map((v, i) => (
              <div key={v.id + i} style={{ background: window.thumbGradient(v.id.charCodeAt(1) + i), borderRadius: 3 }}/>
            ))}
          </div>
        </div>
      </header>
      ...
      <section style={{ borderTop: '1px solid var(--line)', padding: '40px 28px 80px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
            {window.COLLECTIONS.filter(c => c.id !== col.id).slice(0, 4).map(c => (
              <button key={c.id} onClick={() => onNav('collection', c.id)} style={{ textAlign: 'left' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gridTemplateRows: '1fr 1fr', gap: 3, aspectRatio: '4/3', borderRadius: 3, overflow: 'hidden', marginBottom: 10 }}>
                  <div style={{ gridRow: 'span 2', background: window.thumbGradient(c.id.charCodeAt(1)) }}/>
                  <div style={{ background: window.thumbGradient(c.id.charCodeAt(1) + 1) }}/>
                  <div style={{ background: window.thumbGradient(c.id.charCodeAt(1) + 2) }}/>
                </div>
